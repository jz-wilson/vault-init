...
implementation as of v0.4.0. Siblings: [integrations.md](integrations.md) ·
...
- **Critical Constraints (architectural axioms — do not relitigate):**
  ...
  - **Vendored scripts must stay zero-dep.** The `OPERATIONAL` list in `src/init.ts:14` (12 files) is copied into vaults with no `node_modules`; they may only import each other and `node:*` builtins. `src/mcp.ts` is deliberately NOT in that list because it imports `@modelcontextprotocol/sdk`.
...
- **Core Frameworks & Tools:**
  - `bun:test` — 95 test cases across 11 files in `test/`; the only quality gate (`bun test` in CI).
...
- **Published surface:** npm `vault-init` (v0.4.0), GitHub `jz-wilson/vault-init`. `files: ["src", "templates"]` — tests and Taskfile are not shipped.
...
│   └── mcp.ts                # MCP stdio server: vault_search + vault_snapshot (read-only adapters
│                             #   over search.ts/snapshot.ts pure fns)
│
...
│
└── test/                     # 11 bun:test files — foundation, vault, index, search, snapshot,
                              #   nightly, mcp, link-check, skip-subtrees, resolve-inside, init-scaffold
```
...
| `bunx vault-init mcp --dir <vault>` | `src/init.ts` → lazy `import("./mcp.ts")` → `runMcp()` | stdio MCP server |
...
- **Scaffold** (`scaffold()`, `src/init.ts:122`): refuse non-empty target unless `force` → mkdir all dirs via `safeJoin` → write `vault.config.json` → vendor 12 OPERATIONAL scripts → write vault-local `package.json` (script aliases, no deps) → copy pre-commit hook (0o755), `validate.yml`, `nightly.sh` (0o755), hook docs, root docs, SEED-PROMPT → write `.gitignore` (`handoffs/`, `dashboard/status.txt`, `.nightly.log`) → optional example notes → `.gitkeep` any still-empty dir (git won't track empty dirs) → **always** `git init` + `core.hooksPath=.githooks` + initial commit (with fallback identity `vault-init@localhost` if none configured).
...
- **Snapshot** (`buildSnapshot()`, `src/snapshot.ts:56`): walks configured files (default `IDENTITY.md, ALWAYS.md, NEVER.md, AGENTS.md`, budget 1300 tokens) in priority order, packs whole `##` sections while they fit; **first section that would overflow stops ALL further processing** (predictable priority-order cutoff, not best-fit packing; never truncates mid-sentence). Token estimate = `chars/4` — deliberately no tokenizer dep. Intended as a Claude Code SessionStart hook.
- **MCP** (`src/mcp.ts`): `resolveVaultDir()` validates `--dir` (trust boundary: resolve + `vault.config.json` existence). Two read-only tools — `vault_search` (BM25, default limit 10) and `vault_snapshot` (budget override param) — each a thin `{content:[{type:"text",...}]}` wrapper over the same pure fns the CLIs use.
