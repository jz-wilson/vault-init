...
- `bun test` — 95 cases, 11 files. The only gate (CI runs exactly this); no lint/typecheck.
...
- **Two tiers in `src/`**: the `OPERATIONAL` list in init.ts (12 files) is vendored into every
...
- Search: no stemming/stopwords, in-memory corpus per invocation. Snapshot: priority-order
  cutoff (first oversized section stops everything), chars/4 token estimate.
